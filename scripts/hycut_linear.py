"""Batch-based HyCut linear model — sweep over datavault embeddings.
...
def setup_data(dataset_name, model_name):
    ...
    if model_name == 'concat':
        # L2-normalize each and concat
        parts = []
        for m in ['clipvitL14', 'dinov2']:
            ds = load_embeddings(dataset_name, m, REPR_DIR, split="train")
            f = ds.feats.astype(np.float32)
    ...
    # Use tuned graph config if available, else default
    if dataset_name in GRAPH_CONFIG:
        _knn, _metric, _sigma_mode, _sigma_scale = GRAPH_CONFIG[dataset_name]
        from sklearn.neighbors import kneighbors_graph as _knn_graph
        Xk = X
        if _metric == 'cosine':
            Xk = X / np.maximum(np.linalg.norm(X, axis=1, keepdims=True), 1e-12)
        dist = _knn_graph(Xk, n_neighbors=min(_knn, N - 1), mode='distance')
        W_exp = (dist + dist.T) / 2
        W_exp = sp.csr_matrix(W_exp)
        if _sigma_mode != 'none':
            dists = W_exp.data[W_exp.data > 0]
            if _sigma_mode == 'median':
                sigma = float(np.median(dists))
            elif _sigma_mode == 'mean':
                sigma = float(np.mean(dists))
            elif _sigma_mode == 'max':
                maxes = np.array(W_exp.max(axis=1).todense()).flatten()
                sigma = float(np.median(maxes[maxes > 0]))
            elif _sigma_mode == 'per_row_median':
                sigmas = [np.median(W_exp.getrow(i).data) for i in range(N) if W_exp.getrow(i).nnz > 0]
                sigma = float(np.median(sigmas))
            sigma *= _sigma_scale
            W_exp.data = np.exp(-(W_exp.data ** 2) / (2 * sigma ** 2))
            W_exp = (W_exp + W_exp.T) / 2
            W_exp = sp.csr_matrix(W_exp)
        else:
            W_exp.data = np.ones_like(W_exp.data)
        print(f'  graph: knn={_knn} {_metric} {_sigma_mode}×{_sigma_scale}')
    else:
...
def run(objective, data):
    ...
    for step in range(1, steps + 1):
        ...
        if objective == "prcut":
            ...
            # EMA update WITH gradient — so autograd flows through envelope
            alpha_live = ema * alpha_ema + (1 - ema) * P.mean(0).unsqueeze(0)  # (1,K) broadcast to (d,K)
            # Phi is (d, K) without 1/q — lookup by left node's bin
            Phi_bins = compute_ncut_bin_phi(
                q_stars, alpha_live, beta_stars, bin_weights_t, m
            )  # (d, K)
        ...
        with torch.no_grad():
            ...
            # Detach alpha_ema for next iteration
            alpha_ema = (ema * alpha_ema + (1 - ema) * P.detach().mean(0).unsqueeze(0)).detach()
...
for ds_name in DATASETS:
    for model_name in MODELS:
        ...
        if model_name == 'concat':
            emb_path = Path(REPR_DIR) / ds_name / "clipvitL14" / "feats_train.npy"
        else:
